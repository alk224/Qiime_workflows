...
## Reverse complement demultiplexed sequences if necessary
	...
	`adjust_seq_orientation.py -i $seqs -r`
	wait
	mkdir $outdir/original_seqs
	mv $seqs $outdir/original_seqs
	mv $outdir/split_libraries/seqs_rc.fna $outdir/split_libraries/seqs.fna
...
## ITSx filtering (ITS mode only)
	...
	`ITSx_parallel.sh $seqs $itsx_threads`
